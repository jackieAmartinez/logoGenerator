...
class Square extends Shape {
    makeSquare = (color, text, textColor) => {
        ...
        <rect x="70" y="20" width="160" height="160" fill="${color}" stroke="black" stroke-width="3"/>
...
class Circle extends Shape {
    makeCircle = (color, text, textColor) => {
      return `<svg version="1.1"
          ...
          <circle cx="150" cy="120" r="100" fill= "${color}" stroke="black" stroke-width="3" />
...
class Triangle extends Shape {
    makeTriangle = (color, text, textColor) => {
        ...
        <polygon points="150, 18 244, 182 56, 182" fill="${color}" stroke="black" stroke-width="3"/>
        <text x="150" y="125" font-size="60" text-anchor="middle" fill="${textColor}">${text}</text>
        </svg>`;
    };
}
